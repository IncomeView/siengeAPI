...
BASE_URL = "https://api.sienge.com.br/olimpo/public/api/v1/cost-centers"
...
def main():
    ...
    try:
        ...
        df_final = normalize_costCenters(df_raw)

        if df_final.empty:
            log_message("costCenters", "❌ Nenhum dado processado para clientes.")
            return
        df_final["upDate"] = datetime.now(ZoneInfo("America/Sao_Paulo"))
        save_dataframe_insert(df_final, "CostCenters", POSTGRES_SCHEMA)

        # --- Resumo final ---
        elapsed = time.time() - global_start
        log_message("costCenters", "Resumo da execução:")
        log_message(
            "costCenters",
            f"costCenters - status: ✅ ok - início: {started_at} - tempo total: {elapsed:.2f}s - linhas: {len(df_final)}")
    except Exception as e:
        elapsed = time.time() - global_start
        log_message("costCenters", f"❌ Erro ao processar clientes após {elapsed:.2f}s: {e}")
        log_message("costCenters", traceback.format_exc())
